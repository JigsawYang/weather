...
class DisasterController extends CommonController {
    public function __construct() {
        ...
        $this->nav = 'disaster'; //告诉左侧导航激活哪个nav
        $this->disModel = M('featuredisaster');
    ...
    public function index() {
        ...
        //分页相关
        $count = $this->disModel->where($where)->count(); //一共多少个
...
//        print_r($show);die;
        $advs = $this->disModel->limit($Page->firstRow.','.$Page->listRows)->where($where)->order('id desc')->select();
//        print_r($advs);die;
...
//        $this->tab = 'back'; //应该高亮左侧导航的哪一个二级菜单
    ...
    public function edit($id = false) {
        ...
        if ($_POST) {
            ...
            $data['id'] = $id;
            $result = AdvService::dis_save($data);
    ...
    public function add() {
        ...
        if (!$this->disModel->create($data)){   // 对数据进行验证
            $this->error($this->disModel->getError());
        } else {
            $this->disModel->add();
