...
class Pinyin {
    ...
    * Usage Example :
    //echo Pinyin('css',1);
    //echo Pinyin('这是我的博客，欢迎访问'); //默认是gb编
    //echo Pinyin('这是什么',1); //第二个参数随意设置即为utf8编码
    ...
    ***************************************************************************/
    function Pinyin($_String, $_Code='gb2312') {
        ...
        arsort($_Data);
        reset($_Data);
        //$_String='_ '.$_String; //正常使用的时候应该修改。
        //if($_Code != 'gb2312') $_String = _U2_Utf8_Gb($_String);
        if($_Code != 'gb2312') $_String = $this->GetGB2312String($_String);
        $_Res = '';
        for ($i=0; $i<strlen($_String); $i++) {
            $_P = ord(substr($_String, $i, 1)); //每一个字符的编码
            if($_P>160) {
                ...
                $_Res .= $this->_Pinyin($_P, $_Data);
                if (ord(substr($_String, $i+1, 1))>160)$_Res .='';
            }
        }
        return $_Res;
